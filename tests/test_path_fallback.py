...
def test_load_profile_from_json_when_json_exists(tmp_path, monkeypatch):
    ...
    with patch(
        ...
        # Act
        result = _load_or_extract_profile("en")
...
def test_load_profile_extracts_from_primary_md_when_no_json(tmp_path, monkeypatch):
    ...
    with patch(
        ...
        # Act — default language is "de", but only one file exists so backward
        # compatibility kicks in and uses it anyway.
        result = _load_or_extract_profile()

        # Assert
        assert result is mock_profile
        mock_extract.assert_called_once()
        called_source = mock_extract.call_args[0][0]
        assert Path(called_source).resolve() == (bg_dir / "background_english.md").resolve(), (
            f"Expected file path '{bg_dir / 'background_english.md'}', got '{called_source}'"
        )


def test_load_profile_uses_language_specific_md_file(tmp_path, monkeypatch):
    """When multiple .md files exist, the one matching the language is selected."""
    # Arrange
    monkeypatch.chdir(tmp_path)
    bg_dir = tmp_path / "data" / "background_md"
    bg_dir.mkdir(parents=True, exist_ok=True)
    (bg_dir / "background_english.md").write_text("# EN", encoding="utf-8")
    (bg_dir / "background_deutsch.md").write_text("# DE", encoding="utf-8")
...
def test_load_profile_raises_when_multiple_files_and_no_language_match(
    tmp_path, monkeypatch
):
    """When multiple .md files exist and none matches the language, raise FileNotFoundError."""
    # Arrange
    monkeypatch.chdir(tmp_path)
    bg_dir = tmp_path / "data" / "background_md"
    bg_dir.mkdir(parents=True, exist_ok=True)
    ...
    # Act & Assert — language "es" has no matching file here
    with pytest.raises(FileNotFoundError, match="Multiple background Markdown"):
        _load_or_extract_profile("es")
...
def test_load_profile_falls_back_to_examples_when_primary_missing(tmp_path, monkeypatch):
    ...
    fallback_dir = tmp_path / "examples" / "data" / "background_md"
    fallback_dir.mkdir(parents=True, exist_ok=True)
    (fallback_dir / "background_english.md").write_text("# Fallback", encoding="utf-8")
    ...
    with patch(
        ...
        mock_extract.assert_called_once()
        called_source = mock_extract.call_args[0][0]
        assert Path(called_source).resolve() == (fallback_dir / "background_english.md").resolve(), (
            f"Expected file path '{fallback_dir / 'background_english.md'}', got '{called_source}'"
        )
